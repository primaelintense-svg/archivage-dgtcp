...
<head>
    ...
    <title>Changer votre mot de passe — Archivage DGTCP</title>
...
<div class="page">
    <div class="carte">
        ...
        <p class="sous-titre">Pour des raisons de sécurité, vous devez changer votre mot de passe temporaire avant de continuer.</p>

        @if ($errors->any())
            <div class="erreur">{{ $errors->first() }}</div>
        @endif

        <form method="POST" action="{{ route('changerMotDePasse.update') }}">
            @csrf

            <div class="champ">
                <label for="mot_de_passe">Nouveau mot de passe</label>
                <input type="password" id="mot_de_passe" name="mot_de_passe" minlength="8" required autofocus>
            </div>
