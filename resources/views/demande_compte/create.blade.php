...
<head>
    ...
    <title>Demande d'accès — Archivage DGTCP</title>
    ...
    <style>
        :root {
            ...
            --green: #163B2E; --green-deep: #0E2921; --gold: #A9812E; --gold-soft: #D8C48C;
        ...
        * { box-sizing: border-box; }
        ...
        h1 { font-family: 'Source Serif 4', serif; font-style: italic; font-weight: 600; font-size: 20px; text-align: center; margin: 0 0 6px; color: var(--green-deep); }
        .sous-titre { text-align: center; font-size: 12px; color: var(--ink-soft); margin: 0 0 24px; }
        .erreur, .succes { font-size: 13px; padding: 10px 12px; border-radius: 3px; margin-bottom: 20px; }
        .erreur { background: var(--error-bg); border-left: 3px solid var(--error); color: var(--error); }
        ...
        input, textarea {
            ...
            border: 1px solid var(--line); border-radius: 4px; background: var(--parchment); color: var(--ink);
        }
        textarea { resize: vertical; min-height: 70px; }
        input:focus, textarea:focus { outline: 2px solid var(--gold); outline-offset: 1px; border-color: var(--gold); background: var(--surface); }
        button {
            width: 100%; background: var(--green); color: #F6F3EC; border: none; border-radius: 4px;
...
<div class="page">
    <div class="carte">
        <h1>Demande d'accès consultant</h1>
        <p class="sous-titre">Trésor Public du Bénin — un administrateur examinera votre demande</p>

        @if ($errors->any())
            <div class="erreur">{{ $errors->first() }}</div>
        @endif

        <form method="POST" action="{{ route('demandeCompte.store') }}">
            @csrf
            ...
            <div class="champ">
                <label for="motif">Motif de la demande (optionnel)</label>
